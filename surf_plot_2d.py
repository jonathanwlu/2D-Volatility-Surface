...
def plot_symbol(sym):
    with open('spec/VolSurf_S.sql', "r") as f:
        sql = f.read()
    split = sql.split('SPY')
    sql = split[0]
    for i in split[1:]:
    ...
    data = full_df.pivot('Strike', 'TDTE', 'IV').interpolate()
    cnxn.close()

    plt.clf()
    ax = fig.add_subplot(111)

    data.plot(ax=ax, colormap='rainbow', legend='none')
    ...
    plt.axvline(full_df.loc[0, 'sPx'], c='black', ls='dashed', alpha=0.35)

    sm = cm.ScalarMappable(cmap='rainbow', norm=plt.Normalize(vmin=data.columns.min(), vmax=data.columns.max()))
    sm.set_array([])
    plt.colorbar(sm).set_label('TDTE')
